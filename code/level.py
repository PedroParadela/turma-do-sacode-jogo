...
class Level:
	def __init__(self,current_level,surface,criaMapa,mudaMoedas,mudaVida):
		self.display_surface = surface
		self.world_shift = 0
		self.current_x = None
		self.coin_sound = pygame.mixer.Sound('../audio/effects/coin.wav')
		self.stomp_sound = pygame.mixer.Sound('../audio/effects/stomp.wav')
		self.criaMapa = criaMapa
		...
		self.setupDoPlayer(player_layout,mudaVida)
		self.mudaMoedas = mudaMoedas
		...
		self.bg_palm_sprites = self.criaTiling(bg_palm_layout,'bg palms')
		enemy_layout = import_csv_layout(level_data['enemies'])
		self.enemy_sprites = self.criaTiling(enemy_layout,'enemies')
		constraint_layout = import_csv_layout(level_data['constraints'])
		self.constraint_sprites = self.criaTiling(constraint_layout,'constraint')
		self.sky = Sky(8)
		level_width = len(terrain_layout[0]) * tile_size
		self.water = Water(screen_height - 20,level_width)
		self.clouds = Clouds(400,level_width,30)

	def criaTiling(self,layout,type):
	...
	def setupDoPlayer(self,layout,mudaVida):
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				x = col_index * tile_size
				y = row_index * tile_size
	...
	def check_death(self):
		if self.player.sprite.rect.top > screen_height:
			self.criaMapa(self.current_level,0)
			
	def check_win(self):
		if pygame.sprite.spritecollide(self.player.sprite,self.goal,False):
			self.criaMapa(self.current_level,self.new_max_level)
